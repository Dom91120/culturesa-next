...
import { resolveRecurringTarget } from "@/server/services/recurring-booking";
...
// (reserveRecurringInTx / reservePonctuelInTx : déménagés dans services/user-booking —
// partagés avec la liste d'attente, cf. services/waiting-list.)
...
/**
 * Inscription (ou mise à jour) sur la liste d'attente du service : disponibilités par
 * demi-journée + inscription automatique (cf. services/waiting-list). E-mail de
 * confirmation à la PREMIÈRE inscription seulement. Le serveur refuse si le service
 * n'active pas la liste d'attente ou si l'usager n'y a pas accès.
 */
export async function joinWaitingList(serviceId: string, raw: unknown): Promise<Result> {
  const session = await requireUser();
  const parsed = waitlistSchema.safeParse(raw);
  if (!parsed.success) {
    return { ok: false, error: "Indiquez au moins une demi-journée de disponibilité." };
  }
  const svc = await prisma.service.findUnique({
    where: { id: serviceId },
    select: { listeAttente: true },
  });
  if (!svc?.listeAttente) {
    return { ok: false, error: "Ce service ne propose pas de liste d'attente." };
  }
