...
export default async function PointagesPage({
  ...
  const renderSession = (s: DatedSession) => {
    // Comptes anonymisés (RGPD) : on ne sait plus qui c'est, donc personne à faire
    // pointer — ni ligne ni décompte (Dom 2026-09-09).
            ...
            {visibles.map((a, i) => (
              <tr key={`${a.nom}-${a.prenom}-${i}`}>
                <td className="ed-nowrap">
                  ...
                  {a.email && <span className="ed-sub">{a.email}</span>}
                </td>
