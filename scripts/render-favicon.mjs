...
try {
  ...
  await page.evaluate(
    async ({ size, bg }) => {
      ...
      el.id = "favicon-mark";
      // Même recette que .sidebar-mark (30px / .9rem / 700 / C resserré de -0.1em), à l'échelle.
      el.style.cssText = `position:fixed;top:20px;left:20px;width:${size}px;height:${size}px;border-radius:50%;background:${bg};border:${Math.round(size / 30)}px solid #5ab544;box-sizing:border-box;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:${(size * 14.4) / 30}px;letter-spacing:0;line-height:1;z-index:99999`;
      el.innerHTML =
        '<span style="margin-right:-0.1em">C</span><em style="color:#5ab544;font-style:italic">R</em>';
