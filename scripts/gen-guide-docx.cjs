...
async function generate([mdPath, outPath, coverTitle, coverSubtitle, filtre]) {
  ...
  function inlineRuns(text, base = {}) {
    ...
    while (m) {
      ...
      if (m[1]) runs.push(new TextRun({ text: m[2], bold: true, ...base }));
        ...
        runs.push(new TextRun({ text: m[6], ...base })); // lien → texte seul
  ...
  function para(text, opts = {}) {
    children.push(
      new Paragraph({ children: inlineRuns(text, opts.base ?? {}), spacing: P_SPACING, ...opts.p }),
    );
  }
  ...
  children.push(
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [
        new TextRun({ text: "Réservation d'activités culturelles", size: 30, color: "666666" }),
      ],
      spacing: { after: 1400 },
    }),
    new Paragraph({
    ...
    if (
      ...
      const widths = headers.map((_, k) =>
        k === 0
          ? first
          : k === headers.length - 1
            ? totalDXA - first - rest * (headers.length - 2)
            : rest,
      );
      ...
      children.push(
        new Table({
          ...
          rows: [
            new TableRow({
              tableHeader: true,
              children: headers.map((hd, k) => mkCell(hd, true, k)),
            }),
