...
function holidaysInRange(start: string, end: string) {
  ...
  for (let y = ys; y <= ye; y++)
    for (const h of frenchHolidays(y)) if (h.date >= start && h.date <= end) out.push(h);
  return out;
}
...
async function main() {
  ...
  for (const p of periods) {
    ...
    if (rows.length) {
      await prisma.periodHoliday.createMany({ data: rows, skipDuplicates: true });
      phInserted += rows.length;
    }
  }
  console.log(
  ...
  if (kept.length)
    console.log(
      `⚠️ miroirs fériés CONSERVÉS car ils portent des réservations :\n  ${kept.join("\n  ")}`,
    );
}
